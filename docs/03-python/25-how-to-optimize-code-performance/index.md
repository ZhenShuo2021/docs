...
叫 GPT 整理 [Numba 教學](/docs/python/numba-tutorial-accelerate-python-computing) 裡面的優化技巧，標題<u>我自己寫的技巧</u>指的是我在文章中自己手寫的，其他是我自己在教學中附上的文章，再叫 GPT 摘要這些文章的重點。

其實不外乎幾個重點，使用迴圈而不是陣列計算（對於 Numba 的特殊情況）、注意分支處理、注意記憶體和快取連續性、減少記憶體分配和資料移動、分塊計算以提高快取命中率、利用 CPU 指令集。

光我自己寫的就三萬字，累死了，剛寫完沒心情整理啦= =
