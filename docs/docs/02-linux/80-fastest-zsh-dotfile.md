...
⚙️ 設定正確
> 參考 shell 插件開發者 [agkozak](https://github.com/agkozak) 的 shell 設定，筆者就問能有幾個人比 shell 開發者本身設定的更好。
...
> 此安裝方式的具體設定會跟隨我本人的設定進行更新，如果有疑慮請根據[設定段落](#setup-zshrc)操作。
...
現在你已經得到筆者閱讀超過百篇以上文章的精華，並且大量借鑑（抄襲）兩位資深開發者的設定檔，分別是 [narze](https://github.com/narze/dotfiles) 以及 zsh-z 和 zcomet 開發者 [agkozak](https://github.com/agkozak/dotfiles) 的設定檔，這樣還要看網路上的文章只會要你一個一個指令慢慢貼上嗎？或者是半路出家的文章，搞了半天發現作者寫的不是最佳，甚至是錯誤的設定？這裡一行解決而且沒有上述任何問題，覺得有用的話幫筆者的 [repo](https://github.com/ZhenShuo2021/dotfiles) 按星星就很感激了。
...
在網路上搜尋客製化終端機等等關鍵字絕對會查到 Oh-My-Zsh, iTerm2 等等文章，等我們稍微深入一點之後就會看到 Oh-My-Zsh 太慢了開始改用 Zim, Zinit 等等替代原本的 omz，但是這些文章都沒告訴初學者兩個問題：

1. omz, Zim, Zinit 到底在做什麼？
2. 為什麼內建終端機用的好好的要改用 iTerm2？

omz 這些是 <u>**Zsh 插件管理器**</u>，用途是簡化 shell 插件載入過程，iTerm2 則提供分頁、tmux 和 true color 等等內建終端機沒有的功能（不過現在內建終端應該只剩下 true color 和顯示圖片功能不支援）。知道目的後下一步就要討論應該如何<u>**選擇不同的終端和插件管理器**</u>。終端機的部分跳過直接講結論，筆者建議用 WezTerm 直接套用 Github 上面別人的設定檔，或是使用開箱即用的 Warp/Tabby，其餘不推薦，詳細討論請看專文 [各種跨平台終端機實測](../useful-tools/cross-platform-terminal)。

WezTerm 設定檔可以使用筆者自己[稍微改良過的](https://github.com/ZhenShuo2021/wezterm-config)，或者是 KevinSilvester [原版的設定](https://github.com/KevinSilvester/wezterm-config)，提醒一下原版設定需要把預設 shell 從 fish 改為 zsh。

### 如何選擇 Zsh 插件管理器

Zsh 插件的概念最早是由 Oh-My-Zsh 提出的[^first]，因為是先行者，所以是最多人用也是速度最慢的。這些管理器的主要功能不外乎是 clone 插件、載入插件和更新插件。選擇的要點有兩個，方不方便和快不快，速度很直觀，我們也不需要自行測試，已經有人測試插件管理器的速度：
...
5. **下載插件的來源數量**：如 Github repo, 指定 URL, 本機路徑等
6. **補全系統**：自動攔截補全的 compdef 指令，放到最後才一併執行
7. **能不能集中管理 fpath**：支援自動 symlink，避免每個插件都有一個 fpath

問題一其實很好解決[^1]，問題二也不是問題，所以問題只剩下後面五個，既然我們要追求最快的速度那最重要的要求就是第三和第四項，最高的自由度設定，並且支援自動編譯 zwc 檔，其餘都是次要問題。
...
[Eric Nielsen: Zim Speed](https://github.com/zimfw/zimfw/wiki/Speed) 不是說這些是最快的幾個嗎？讓筆者仔細說明，Zim 要求把插件放到獨立的 .zimrc 文件統一載入，這樣會造成我們無法對每個插件手動進行管理，尤其個別設定是否使用 zsh-defer 這個延遲載入的工具插件。zsh-defer 的原理是延遲載入讓 shell 在沒有工作時才載入以降低實際等待時間，然而 Zim 統一載入的特性會導致我們無法獨立控制每個插件是否啟用 zsh-defer，只能設定全開或全關，全開時會因為無法達成 Zsh 補全系統要求的載入順序造成部分補全系統啟用失敗，<u>**全關時不使用延遲載入，這些插件管理器就不是比誰快，而是慢和更慢**</u>，zsh4humans 也有類似問題只是比較輕微（因為他真的很快）。

至於 Zinit，他內建的 Turbo mode 就是延遲載入，但是本體的載入速度太慢是他最大的缺點，第二大的缺點是語法過度複雜。
...
2. 不用搞 gnu stow，不需要像 dotbot 需要額外安裝，沒有複雜的語法，一行指令自動安裝
3. 內建實用插件，不多不少剛剛好
4. 極簡風格，不搞花花綠綠的分散注意力
...
網路上的中文文章一堆設定錯誤，沒有啟用延遲載入、補全設定錯誤，筆者甚至能把正確拿來當賣點，如果按照其他中文文章設定，高機率有某些功能設定錯誤。
...
讀者可能看過 [Ubuntu 安裝 Zsh + Oh My Zsh + Powerlevel10k 與各種插件](https://www.kwchang0831.dev/dev-env/ubuntu/oh-my-zsh)，才安裝兩三個插件對初學者來說已經滿頭大汗了，這不是文章的問題而是 omz 的問題，使用現代插件管理器可以更輕鬆的安裝插件，在 zshrc 後面接著貼上：
...
Bang! 設定完了，你已經成功安裝 10 個插件而且不需要貼上一堆有的沒的指令。

> [zsh-defer](https://github.com/romkatv/zsh-defer) 和 [antidote 作者](https://github.com/mattmc3/zsh_unplugged) 都表示不建議使用 zsh-defer 載入插件，但是筆者實際測試沒有發現什麼問題，功能都正常運作，而且也就是為了避免問題才特意避開 Zim 這種無法單獨控制 defer 的插件管理器。

### 啟用補全系統

這裡筆者真的要說自己在種樹了，就沒有一篇文章能好好說明 Zsh 補全系統是怎麼運作的，結論很簡單，資源很難找：

1. 設定 fpath
2. 設定 zstyle
3. 執行 compinit
4. 設定 zstyle（如果有其餘的 zstyle 必須在 compinit 後執行的話）
5. 執行 functions requires compdef
6. 執行 zsh-syntax-highlighting > zsh-autosuggestions

用人話解釋這些是什麼，fpath 是 Zsh 才有的路徑，用於設定補全腳本和函數的路徑（而不是腳本或二進制程式，這兩個的路徑是 PATH）；zstyle 有很多功能，最顯而易見的就是設定補全的樣式；接著 compinit 啟用補全系統，再執行需要依賴補全系統的指令，最後執行插件開發者自己說應該在最後載入的插件。
...
恭喜又完成超多優化。如果沒安裝 uv 最後一行可以刪掉，如果你是 Python 開發者那麼筆者強烈建議使用 uv，請參考筆者的文章 [Python UV 教學，最佳虛擬環境管理工具](../python/python-uv-complete-guide)。
...
如果沒安裝過 brew 那應該也還沒安裝 nerd font，使用此指令安裝：
...
所有 brew 設定都已經在上面的文件中設定完成，完全不需要費心調整。
...
前面說到搞笑演員拿 time 指令跑 10 次就說這是效能測試，首先好歹拿個 hyperfine 就可以一鍵生成統計數據，筆者使用 hyperfine 執行 100 次測試統計結果都沒有到非常穩定了，跑 10 次不是來搞笑的嗎？而且 [zsh-bench](https://github.com/romkatv/zsh-bench/) 早在 2021 就說了測試 `zsh -i -c exit` 毫無意義[^meaningless]，功課不做就開始寫文章誤導他人。搞笑演員至少還可以搞笑，甚至還有很多文章使用 Zinit 結果[不使用 turbo mode](https://github.com/zimfw/zimfw/wiki/Speed)，我無話可說。
