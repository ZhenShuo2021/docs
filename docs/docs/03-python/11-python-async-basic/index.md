...
keywords:
  ...
  - 異步
last_update:
  date: 2024-11-15T10:20:00+08:00
  author: zsl0621
---
...
來自 [前一篇文章](/docs/python/python-multitasking-threading-processing-coroutine) 的說明：協程是一個人在<u>**等待**</u>某項工作（如等待網路回應）時，先<u>**切換**</u>去處理其他工作，線程和進程則是多個人同時工作[^diff]。如果下面的對比看完有點不清楚，記得這句話至少理解不會歪掉。

[^diff]: 註：Python 中的線程被 GIL 鎖住雖然有很多人但是一次只有一人工作。

- 控制方式差異  
對比線程和進程，協程是<u>**完全不同概念，無法類比**</u>，協程由「程式本身」管理，是一種允許在單一線程內實現異步執行的程式「物件」，後兩者是由作業系統管理的「執行單元」，由「作業系統」[控制](https://discuss.python.org/t/python-context-switching-how-is-it-done/8635)。
...
- 更具體說明  
協程透過 <u>**事件迴圈 (event loop)**</u> 調度。當程式執行到 await 語句（等待某項工作）時，事件迴圈會切換（切換去處理其他工作）到下一個可執行的任務，藉此利用等待時間執行其他任務，因此特別適合 I/O bound 的任務，例如網路請求、檔案操作等任務。
...
- 文章閱讀：[異步技術：徹底理解Async、Await與Event Loop](https://medium.com/@ackerley19/%E7%95%B0%E6%AD%A5%E6%8A%80%E8%A1%93-%E5%BE%B9%E5%BA%95%E7%90%86%E8%A7%A3async-await%E8%88%87event-loop-3943a4ec9814)，少數說明清楚並且正確的文章

## 協程程式的組成元件

> 通常筆者會從高階使用開始介紹，最後才是低階組成，然而異步的特殊性讓其需要從低階開始介紹，如果從高階用法開始介紹會造成「可以動但不知道做了什麼」的問題。

經由上述介紹我們可以知道協程不是一個活的線程或進程，是「程」這個翻譯帶來誤會。一個 Python 非同步程式需要以下幾個元件組成：

1. 事件迴圈 (Event-loop)：非同步的關鍵核心，負責協調不同任務。事件迴圈不斷檢查是否有新任務並管理它們的進入與退出。
2. 任務 (Task)：事件迴圈中工作執行的單位，運行協程的載體，負責追蹤協程的執行狀態、例外處理，繼承自 Future。
3. 協程 (Coroutine)：使用 `async def` 定義的函式，以及所有可以暫停與恢復執行的物件。
4. await 關鍵字：執行到此行會自動讓出控制權，用來等待一個 [awaitable](https://docs.python.org/3/glossary.html#term-awaitable) 物件完成。

(還有不常用到的五：未來物件 (Future)，代表未完成的計算結果，可被設定為完成或取消。)

> awaitable 物件定義: 協程或是有 `__await__` 方法的物件。
...
如果看舊版文章會發現使用 `yield` 的生成器函式、使用 `@types.coroutine` 裝飾器的函式都屬於協程。現在沒有必要使用這些用法，看過就好。

協程分為兩種：協程函式和程物件，當我們使用 `async def` 定義一個函式，他是協程函式；當我們實例化這個函式，他無法被直接執行，而是返回一個協程物件。
...
雖然看起來很麻煩，但是實際使用時我們不會這樣手動慢慢建立，使用高階函式完成可以簡化成以下輕鬆完成：
...
def run(main, *, debug=None):
    ...
    try:
        ...
        # 將會被隱含調度為asyncio.Task運作，筆者範例使用顯式轉換成asyncio.Task
...
async def main():
    tasks = [my_coroutine(1) for _ in range(20)]
...
### 最大同時並行數量範例
上面一堆 1 滿無聊的對吧，這裡使用 Semaphore 鎖住最大任務數量，並且由此範例說明執行時間。
...
## 相關語法和函式
本章節列出常用的語法和函式。學習協程的障礙在於不清楚有哪些物件以及不知如何執行，前面的文章已經說明相關物件，接下來列出相關函式讓你快速知道如何調用，不用找文檔找到懷疑人生。
...
每個想取代 asyncio 的套件都會先說他的效能多爛，如果筆者沒測試過，但是放上整理的一些套件：
...
查協程會看到一張圖，多個方塊代表任務，協程從水平連接變成折線連接，圖放了沒有任何說明，圖爛到連路上隨便一個大學生來做都不如，這圖我不相信初學者看得懂，橫軸縱軸都沒標誰知道你在講什麼？圖就不放了怕被扁。

還有更荒謬的，把 multi-processing 標示成多處理器，當初看到直接關網頁，撰文找資料時才發現應該是 typo，還好不是真的這麼荒謬。有的還會講故事說明，一句話「需要等待時主動切換任務」不就結束了，偏偏要寫一個故事，讀完也沒有更深刻體悟，第一次學的看到這故事越看越混亂。
