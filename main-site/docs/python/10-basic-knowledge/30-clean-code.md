...
要強調的是很多文章都把這些原則介紹的像是至高無上的真理一樣，**這絕對是錯誤的**，正確的是透過這些原則幫助我們寫出「可讀性高、可維護、可擴展」的程式碼而不是追求原則，[更不是盲目追求 clean code](/blog/do-not-write-clean-code#unclean-code)。
...
DRY (Don't Repeat Yourself) 很多人會這樣解釋：同樣的程式碼重複超過三次就抽象成函數，雖然這個說法不算錯一開始學也可以這樣用，但是詳情請見[別寫乾淨的程式](/blog/do-not-write-clean-code)。
...
2. 濫用 DRY 會造成更大問題，請看[別寫乾淨的程式](/blog/do-not-write-clean-code)。
