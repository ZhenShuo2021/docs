...
分支操作主要有以下五大指令，扣掉複雜的 rebase 以外其餘根本不需要每個寫成一篇文章來介紹。本文只會有本地不會有遠端操作，遠端操作的第一篇文章要從[遠端儲存庫設定](/core/remote-setup)開始。
...
> 看不懂 pathspec？請見[看懂文檔](/basic/read-git-docs)。
...
兩者的選擇端看是否需要保存分支結構。合併時可能會遇到衝突，這需要手動解決，解決方式請看 [如何解決合併衝突](/basic/keyword#進階)。
...
為了搞懂 rebase 看了很多文章，最後濃縮成這兩句話[^compress]，使用教學請見本教學的 [使用變基 Rebase 合併分支](/core/rebase)。
...
互動式變基 (interactive rebase) 使用變基的原理實現對提交歷史進行任意修改，同時**使用方式非常簡單**，請見[使用互動式變基任意修改提交歷史](/core/interactive-rebase)。
...
變基時可能會遇到衝突，解決方式請見[如何解決合併衝突](/basic/keyword#進階)。
...
到這邊你已經熟悉 Git 的所有常用指令了，只是需要一點時間熟悉，下一步請快轉到[遠端儲存庫設定](/core/remote-setup)，進階文章初學者用不到建議暫時跳過以免越看越亂。
