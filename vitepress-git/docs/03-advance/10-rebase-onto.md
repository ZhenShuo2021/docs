...
這個文檔從我還不熟 Git 的時候開始寫，開宗明義就說**使用官方翻譯而不是自己造詞**，現在回頭看我最初講的話是對的。在蒐集 rebase onto 資料時發現很多文章寫了「onto 之後的參數是 `<new base-commit> <current base-commit>`」，這就是明顯的亂造詞問題，後來發現所有的錯誤用法都來自於[同一篇文章](https://git-tutorial.readthedocs.io/zh/latest/rebase.html)，也可以算是一種 error propagation 吧。
